...
def sauvegarder_excel(resume, nom_fichier='facture.xlsx'):
    ...
    for invoices in list_invoices:
        contenus = lecteurs.lire_texte_simple(f'invoice24/{invoices}')

        # Traiter la facture
        parsers = FactureParser(contenus[0])
        resumes = {
            'informations_client': parsers.obtenir_infos_client(),
            'articles': parsers.obtenir_articles(),
            'totaux': parsers.obtenir_totaux(),
            'notes': parsers.obtenir_notes()
        }
        pairs = dict(pair.split('=') for pair in contenus[0].split('&'))
    ...
    # Ajouter les totaux
    row_totaux = 8 + len(resumes['articles'])
    worksheet.cell(row=row_totaux, column=1, value='Total HT:')
    worksheet.cell(row=row_totaux, column=2, value=resumes['totaux']['total_ht'])
    worksheet.cell(row=row_totaux + 1, column=1, value='Montant payé:')
    worksheet.cell(row=row_totaux + 1, column=2, value=resumes['totaux']['montant_paye'])

    # Ajouter les notes
    row_notes = row_totaux + 3
    worksheet.cell(row=row_notes, column=1, value='Notes:')
    worksheet.cell(row=row_notes, column=2, value=resumes['notes']['notes_communes'])
    worksheet.cell(row=row_notes + 1, column=1, value='Remarques:')
    worksheet.cell(row=row_notes + 1, column=2, value=resumes['notes']['notes_pied'])
...
for invoice in list_invoices:
      ...
      sauvegarder_excel(resume, f"{client}-{date}.xlsx")