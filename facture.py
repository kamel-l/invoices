...
class StylesFacture:
    ...
    remplissage_entete = PatternFill(
        start_color=BLEU_CLAIR,
        end_color=BLEU_CLAIR,
        fill_type='solid'
    )

    remplissage_total = PatternFill(
...
class FormateurFacture:
    ...
    def appliquer_style_entete(self, worksheet, row: int, col_start: int, col_end: int):
    ...
    def formater_facture(self, worksheet):
        ...
        # Informations client (ligne 2-5)
        self.appliquer_style_entete(worksheet, 2, 1, 6)
        for row in range(3, 6):
            self.appliquer_style_ligne(worksheet, row, 1, 6)

        # Section articles
        self.appliquer_style_entete(worksheet, 7, 1, 6)
        derniere_ligne = worksheet.max_row

        for row in range(8, derniere_ligne - 4):
            self.appliquer_style_ligne(worksheet, row, 1, 6)

        # Section totaux
        for row in range(derniere_ligne - 3, derniere_ligne - 1):
    ...
    def charger_fichiers(self) -> None:
        ...
        if not self.dossier_source.exists():
            raise FileNotFoundError(
                f"Le dossier {self.dossier_source} n'existe pas"
            )

        fichiers_excel = list(self.dossier_source.glob("*.xlsx"))
        if not fichiers_excel:
            logger.warning(
                f"Aucun fichier Excel trouvé dans {self.dossier_source}"
            )
            return
...
def main():
    """Fonction principale"""
    try:
        dossier_source = 'invoices_excel0/YOUSEF'
        fichier_destination = 'YOUSEF.xlsx'
