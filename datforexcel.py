...
for invoice in list_invoices:
      ...
      clients = client.replace("%20", '_')
      Ninvoice = invoice.replace(".dat", '')

      # Créer le dossier de destination s'il n'existe pas
      download_path = Path("invoices24_downloaded")
      download_path.mkdir(parents=True, exist_ok=True)

      # Construire le chemin du fichier Excel
      nom_fichier = f"{clients}-{date}.xlsx"
      chemin_complet = download_path / nom_fichier

      # Sauvegarder en Excel
      sauvegarder_excel(str(chemin_complet))
      print(f"Facture sauvegardée avec succès : {nom_fichier}")
